                                   ...
                                   (let [comparisons      (compare-coordinate-systems comparison rotation)
                                     ...
                                     (if (seq scanner-positions-filtered)
                                       (let [scanner-position (key (apply max-key val scanner-positions-filtered))]
                                         (normalize-coords scanner-position rotation))
                                       nil))))]
        (println "Comparing " comparing-key "|" "To be compared" to-be-compared)
        (if (empty? normalized)
          (recur comparison (first to-be-compared) (concat (rest to-be-compared) [comparing-key]))
          (recur (set/union comparison (into #{} (first normalized))) (first to-be-compared) (rest to-be-compared)))))))
